...
void main() {
  ...
  group('Battery Provider Integration Tests', () {
    test(
      'batteryProvider should load the level from repository on creation',
      () async {
    ...
    test(
      'batteryStateProvider should emit the correct state from repository',
      () async {
        // Arrange
        when(
          mockRepository.batteryState,
        ).thenAnswer((_) async => BatteryState.charging);
        final container = createContainer();

        // Act
        final stateAsyncValue = await container.read(
          batteryStateProvider.future,
        );
    ...
    test(
      'thresholdProvider should read the baseline from SettingsService',
      () async {
        // Arrange
